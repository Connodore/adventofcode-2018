...
/*****************************************************************************/
// Returns an unordered_map of each guard's ID to an unordered_map counting the minutes each guard slept

template <class RecordIt>
std::unordered_map<int, std::unordered_map<int, int>>
get_sleeptimes(RecordIt first, RecordIt last)
{
    std::unordered_map<int, std::unordered_map<int, int>> sleeptimes{};
...
find_max(SleepIt first, SleepIt last)
{
    // Find the minute each guard most commonly slept on
    std::unordered_map<int, std::pair<int, int>> max_minutes{};
    std::transform(first, last, std::inserter(max_minutes, end(max_minutes)), [](const auto &guard) {
        auto &[id, minutes] = guard;

        auto max_minute = *std::max_element(begin(minutes), end(minutes), [](const auto &lhs, const auto &rhs) {
            return lhs.second < rhs.second;
        });

        return std::pair{id, max_minute};
    });

    //
    auto [id, max_minute] = *std::max_element(begin(max_minutes), end(max_minutes), [](const auto &lhs, const auto &rhs) {
        auto [_1, max_minute1] = lhs;
        auto [_2, max_minute2] = rhs;

        return max_minute1.second < max_minute2.second;
    });
